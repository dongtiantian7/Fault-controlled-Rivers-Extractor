...
namespace test
{
    class Matching
    {
        ...
        private void InitVisit(GraphStruct subG)
        {
            ...
            {
                int gId = subG.VexList[i].Data.PID;
                this.visit[gId] = true;
            }
        }
                ...
                for (int j = 1; j < pGraph.VexNum; j++)
                {
                    ...
                    if (flag)
                    {
                        ...
                        { 
                            ...
                            //数据图G 回溯
                            int lastDataNodeId = subGraph.VexList[subGraph.VexNum - 1].Data.PID;
        ...
        {
            int lastNodeId = this.dataGraph.GetIndex(lastNode.Data);
        ...
        private bool SemanticMatch(GraphStruct subG)
        {
            ...
            for (int i = 0; i < subG.VexNum;i++ )
            {
                 if (pGraph.VexList[i].Data.NodeType != 0)
                {
                    //节点的语义匹配
                    if ((subG.VexList[i].Data.NodeType != pGraph.VexList[i].Data.NodeType) && (subG.VexList[i].Data.NodeType != 11))
                        return false;
                    //if (subG.VexList[i].Data.NodeType * pGraph.VexList[i].Data.NodeType < 0) 
                    //    return false;
                }
        ...
        private bool SeMatchNode(VexNode gNode, VexNode pNode)
        {
            ...
            for (AdjNode gAdjNode = gNode.FirstAdj; gAdjNode != null; gAdjNode = gAdjNode.Next)
            {
                for (AdjNode pAdjNode = pNode.FirstAdj; pAdjNode != null; pAdjNode = pAdjNode.Next)
                {
                    ...
                    {
                        gEdgeValue = gAdjNode.EdgeValue;
                        pEdgeValue = pAdjNode.EdgeValue;
